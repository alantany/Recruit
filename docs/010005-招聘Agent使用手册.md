...
- `docs/010000-docs文档索引.md`
  全部 `docs` 文档的编号索引与导读（正文文档为 `010001`～`010008`）
...
**`deploy-windows.bat`、`run.bat` 与 `run.sh`：**
...
| `run.bat`（Windows 另一台机器） | **日常**：`git pull origin main`、`npm install`，然后 **`npm run agent:search-interaction-loop`（主线 A）** | **想跑 Agent 时用**（从远程同步后再跑） |
| `run.sh`（macOS/Linux 开发机） | **日常**：**不** `git pull`，仅 `npm install` + 主线 A（以本地代码为准） | **想跑 Agent 时用** |
...
- `docs/010002-互动区LLM提示词.md` 的 System Prompt 与 JSON 字段是否一致
...
更完整的需求级说明见 `docs/010008-招聘Agent需求文档.md` §11.7。
...
1. `docs/010008-招聘Agent需求文档.md`
2. `docs/010002-互动区LLM提示词.md`
3. `docs/010005-招聘Agent使用手册.md`
